...
@section('dashboard_panels')
              ...
              <header class="panel-heading">
                  SERVICE EQUIPMENT TYPE MANAGEMENT 
             <span class="tools pull-right">
              ...
              <section class="panel">
                      <header class="panel-heading" style="background: white !important;  border-color: solid grey; color:grey;">
                             <b><i>View/ Edit/ Delete: service equipment types. </i></b>
                      </header>
                  ...
                  <table class="table table-bordered table-striped table-condensed cf" style="text-align:center;">
                        <thead class="cf">
                          <tr>
                            ...
                            <th style="text-align:center;">Service Equipment Type</th>
                          ...
                          <tr>
                            ...
                            <td>
                              <a style="display:inline;" data-toggle="modal" href="#myModal" class="link_id" id="{{$row['id']}}" name="edit_link" typeName="{{htmlentities($row['name'])}}">
                                {{ Form::button('<i class="fa fa-pencil"></i>', array('class' => 'btn btn-primary btn-xs', 'title'=>'Edit Type')) }}
                              </a>
                              {{ Form::open(array('method' => 'post','id'=>'myForm'.$row['id'].'','style'=>'display:inline; margin:0px; padding:0px;', 'route' => array('parts/deleteEquipType', $row['id']))) }}
    ...
    <script type="text/javascript">
     ...
     $('#submit_part_type').click(function(){
        if ( $('#_name').val() == ''){
          ...
          $.ajax({
            url: "{{URL('ajax/updateEquipType')}}",
              data: {
                'id' : $("#_type_id").val(),
                'name' : $("#_name").val()
              },
              success: function (data) {
                alert("Equip. Type successfully updated!");
                location.reload();
            },
          });
        }
