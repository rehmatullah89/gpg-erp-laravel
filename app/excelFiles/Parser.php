...
*/
  function _convert($token)
    ...
    elseif(preg_match("/^([A-I]?[A-Z])(\d+)$/i",$token))
        {
        return($this->_convert_ref2d($token));
        }
    // match ranges like A1:B2
    elseif(preg_match("/^([A-I]?[A-Z])(\d+)\:([A-I]?[A-Z])(\d+)$/i",$token))
        {
    ...
    elseif(preg_match("/[A-Z0-9À-Ü\.]+/i",$token))
        {
...
*/
  function _convert_number($num)
    ...
    if ((preg_match("/^\d+$/i",$num)) and ($num <= 65535)) {
...
*/
  function _convert_range2d($range)
    ...
    if(preg_match("/^([A-I]?[A-Z])(\d+)\:([A-I]?[A-Z])(\d+)$/i",$range)) {
        //list($cell1, $cell2) = split(':', $range);
		list($cell1, $cell2) = explode(':', $range);
        }
    elseif(preg_match("/^([A-I]?[A-Z])(\d+)\.\.([A-I]?[A-Z])(\d+)$/i",$range)) {
        //list($cell1, $cell2) = split('\.\.', $range);
		list($cell1, $cell2) = explode('\.\.', $range);
        }
    else {
...
*/
  function _cell_to_rowcol($cell)
    {
    preg_match('/(\$)?([A-I]?[A-Z])(\$)?(\d+)/i',$cell,$match);
