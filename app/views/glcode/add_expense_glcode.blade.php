...
@section('dashboard_panels')
             ...
             <!-- ////////////////////////////////////////// -->
              {{ Form::open(array('before' => 'csrf' ,'url'=>route('glcode/add_expense_glcode'), 'files'=>true, 'method' => 'post')) }}
              ...
              <section id="no-more-tables" >
                  <table class="table table-bordered table-striped table-condensed cf" id="myTable" style="text-align:center;">
                        <tbody>
                          ...
                          <td>
                            {{ Form::select('_parent_id',array(''=>'Select Parent')+$gcodes,'', array('class' => 'form-control dpd1', 'id' => '_parent_id', 'required')) }}
                            </td>
                          </tr>
                          <tr>
                          <th style="text-align:center;">Expense Gl-Code Type[{{ HTML::link('glcode/new_type/', 'Add New' , array('class'=>'btn btn-link btn-xs'))}} ]:*</th>
                          <td>
                            {{ Form::select('_gpg_expense_gl_type_id',array(''=>'Select Type')+$gltypes,'', array('class' => 'form-control dpd1', 'id' => '_gpg_expense_gl_type_id', 'required')) }}
                            </td>
                          </tr>
                          
                          <tr>
                          <th style="text-align:center;">Expense  GL-Code#:*</th>
                          <td>{{ Form::input('number','expense_gl_code','', array('class' => 'form-control dpd1', 'id' => 'expense_gl_code', 'required')) }}</td>
                          </tr>
                          <tr class="qntity">
                          ...
                          <td>
                            {{ Form::select('_status',array('A'=>'Active','B'=>'Blocked'),'', array('class' => 'form-control dpd1', 'id' => '_status', 'required')) }}
                            </td>
                          </tr>
                          <tr>
                          <th style="text-align:center;">Exclude form OH Calculation:</th>
                          <td>
                            {{ Form::checkbox('_exclude_from_oh','1', array('class' => 'form-control dpd1', 'id' => '_exclude_from_oh')) }}
                            </td>
                          </tr>
                          <tr>
                    ...
                    {{ Form::submit('Add Expence GL-Code', array('class' => 'btn btn-success')) }}
              </section>
              </div>
              </div>
              {{ Form::close() }}
