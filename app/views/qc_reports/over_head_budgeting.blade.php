...
@section('dashboard_panels')
 <!-- page start-->
  ...
  {{ Form::open(array('before' => 'csrf','id'=>'default','class'=>'form-horizontal' ,'url'=>route('qc_reports/over_head_budgeting'), 'files'=>true, 'method' => 'post')) }}
...
<script>
      ...
      $(function() {
          $('#default').stepy({
              ...
              var step = '{{$step}}';
              if (step == '0'){
                    if($('#uploadFile').val() == "" || $('#SDate').val() == "" || $('#EDate').val() == ""){
                      alert("Please select file & Dates first."); 
                      return false;  
                    }else{
                      conf = confirm('WARNNING: The Recods from selected dates Will be Deleted from database!!');
                      if(conf == false){
                        return false;
                      }else{
                         $('#default').submit();
