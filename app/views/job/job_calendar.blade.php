             ...
             <span class="tools pull-right">
              ...
              <section class="panel">
                  <header class="panel-heading" style="background: white !important;  border-color: solid grey; color:grey;">
                    <b><i>View Job data grouped by customer/type/employee with different filters. </i></b>
                  </header>
              </section>
             <!-- ////////////////////////////////////////// -->
              ...
              {{ Form::open(array('before' => 'csrf' ,'url'=>route('job/'.$uriSegment), 'files'=>true, 'method' => 'post')) }}
               <section id="no-more-tables" style="padding:10px;" mySection="hide_n_show">
                  <table class="table table-bordered table-striped table-condensed cf" id="mytable" align="center">
                    <tbody>
                      <tr>
                        <td>{{Form::select('group_view',array(''=>'Normal View','customer'=>'Group by Customer','job_type'=>'Group by Job Type','assigned_to'=>'Group by Employee'), null, ['id' => 'group_view', 'class'=>'form-control m-bot15'])}}</td>
                        <td>{{Form::select('jobTypeFilter',array(''=>'Job Type Filter')+$gpg_job_type, null, ['id' => 'jobTypeFilter', 'class'=>'form-control m-bot15'])}}</td>
                        <td>{{Form::select('techniciansFilter',array(''=>'Technicians Filter')+$gpg_employee, null, ['id' => 'techniciansFilter', 'class'=>'form-control m-bot15'])}}</td>
    ...
    $('#calendar').fullCalendar({
        ...
        events: [<?php
            foreach ($events as $key => $value) {
                 echo '{title:"'.$value['title'].'",url:"'.$value['url'].'",start: "'.date('Y-m-d',strtotime($value['start'])).'"},';
            }        
        ?>]
    });
}();
