...
include_once(app_path() . '/excelFiles/Parser.php');
include_once(app_path() . '/excelFiles/BIFFwriter.php');
...
class Worksheet extends BIFFwriter
{
    ...
    */
    function _write_url_internal($row1, $col1, $row2, $col2, $url, $str, $format = 0)
    {
        ...
        // Strip URL type
		//echo $url;
        $url = preg_replace('[^internal:]' , '', $url); // modified by wajhe
		//$url = preg_replace('s[^internal:]' , '', $url); original 
		
    
        // Write the visible label
        if($str == '') {
            $str = preg_replace(array('[\(]','[\)]','[!A1]'), '', $url); // modified by wajhe
		//$str = $url; original 
        }
        $str_error = $this->write_string($row1, $col1, $str, $format);
        if ($str_error == -2) {
