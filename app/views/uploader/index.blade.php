...
@section('dashboard_panels')
 <!-- page start-->
  ...
  {{ Form::open(array('before' => 'csrf','id'=>'default','class'=>'form-horizontal' ,'url'=>route('uploader/index'), 'files'=>true, 'method' => 'post')) }}
                            <fieldset title="Step1" class="step" id="default-step-0">
                                  ...
                                  <fieldset title="Step 2" class="step" id="default-step-1" >
                                      ...
                                      @if(isset($jcostopt_arr) && !empty($jcostopt_arr))
                                       <label class="col-lg-2 control-label">Select Job Category*:</label>
                                          <div class="col-lg-4">
                                               {{Form::select('jobCat', $job_cat_arr,'', ['id' => 'jobCat', 'class'=>'form-control m-bot15'])}}
                                          </div><br/><br/><br/><br/><br/>
                                  ...
                                  <fieldset title="Step 3" class="step" id="default-step-2" >
                                      ...
                                      <label class="col-lg-2 control-label">Select Job Category*:</label>
                                          <div class="col-lg-4">
                                               {{Form::select('job_cat', $job_cat_arr,'', ['id' => 'job_cat', 'class'=>'form-control m-bot15','readOnly'])}}
                                          </div><br/><br/><br/><br/><br/>
                                      @foreach($jcostopt_arr as $val)
                                      <div class="form-group">
                                          <label class="col-lg-2 control-label">File Field {{$i}}:</label>
...
<script>
          ...
          $('#default').stepy({
              ...
              if (step == '0'){
                    if($('#uploadFile').val() == ""){
                      ...
                      conf = confirm('Note: If want to ignore some column(s) please do not select the database field for it.');
                      if(conf == false){
                        return false;
                      }else{
                         $('#default').submit();
    ...
    $(document).ready(function() {
      ...
      $('#default-next-1').click(function(){
        ...
        var i=1;
        $('#job_cat').val($('#jobCat :selected').val());
        var flag=0;
        if(arr != ''){
          while(i<arr.length){
            $('#db_field_'+i).val($('#field_db_'+i).val());
            i++;
            if ($('#field_db_'+i).val() == 'job_num'){
              flag=1;
            }
          }
          if(flag == 0){
            alert('Please Select a DB Field with Job Number!');
            $('#default-next-0').click();
          }
        }
      });
