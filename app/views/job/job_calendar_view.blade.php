              ...
              {{ Form::open(array('before' => 'csrf' ,'url'=>route('job/'.$uriSegment), 'files'=>true, 'method' => 'post')) }}
               <section id="no-more-tables" style="padding:10px;" mySection="hide_n_show">
                  <table class="table table-bordered table-striped table-condensed cf" id="mytable" align="center">
                    <tbody>
                      <tr>
                        <td>{{Form::select('group_view',array(''=>'Normal View','customer'=>'Group by Customer','job_type'=>'Group by Job Type','assigned_to'=>'Group by Employee'),'customer', ['id' => 'group_view', 'class'=>'form-control m-bot15'])}}</td>
...
<script type="text/javascript">
   $('#group_view').change(function(){
    var vl = $(this).val();
    if (vl == '') {
      location.href="{{URL::to('job/job_calendar')}}";
    }
  });
  var Script = function () {
