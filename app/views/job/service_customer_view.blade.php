             ...
             <span class="tools pull-right">
              ...
              <section class="panel">
                <header class="panel-heading" style="background: white !important;  border-color: solid grey; color:grey;">
                  <b><i>Service Job's Customer Listing </i></b>
                </header>
                            ...
                            <tbody>
                              <tr>
                                ...
                                <td colspan="2">
                                  ...
                                  {{ HTML::link("job/serviceCustomerExport?".http_build_query(array_filter(Input::except('_token', 'page'))), 'Export as Excel' , array('class'=>'btn btn-success'))}}
                                </td>
                                </tr>
                              </tbody>
                          </table>
              ...
              <?php $index=1;?>
                    @foreach($qry_data as $key=>$arr_data)
                      <tr>
                        ...
                        <td data-title=":">{{$arr_data['count']['total']}}</td>
                        <td data-title=":" class="hidden-phone">{{'-'}}</td>
                        <td data-title=":" class="hidden-phone">{{$arr_data['count']['incomplete']}}</td>
                      </tr>
                    <tr id="hideme_{{$index}}"><td colspan="5">
                          <table class="table table-bordered table-striped table-condensed cf">
                           @foreach($arr_data as $k=>$val)  
                           @if($k != 'count' && $k != 'cus_name' && $k != 'service_types')    
                            <tr><th>Customer</th><td>{{$k}}</td></tr>
                            <tr><th>Job Number</th><th>Contract Number</th><th>Location</th><th>Task</th></tr>
                            @foreach($val as $k2=>$v2)
                              @if($k2 != 'location' && $k2 != 'cus_name' && $k2 != 'count')
                              <tr>
                                <td>{{$k2}}</td><td>{{$v2['contract_number']}}</td><td>{{$v2['location']}}</td><td>{{$v2['task']}}</td>
                              </tr>
                              @endif
                            @endforeach
                          @endif  
