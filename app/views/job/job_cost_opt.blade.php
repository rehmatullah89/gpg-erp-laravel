...
@section('dashboard_panels')
 <!-- page start-->
  ...
  {{ Form::open(array('before' => 'csrf','id'=>'default','class'=>'form-horizontal' ,'url'=>route('job/job_cost_opt'), 'files'=>true, 'method' => 'post')) }}
                            <fieldset title="Step1" class="step" id="default-step-0">
                                      <legend> </legend>
                                      <div class="form-group">
                                          <label class="col-lg-2 control-label">Start Date:</label>
                                          <div class="col-lg-10">
                                              {{ Form::text('SDate',(isset($SDate)?$SDate:''), array('class' => 'form-control form-control-inline input-medium default-date-picker', 'id' => 'SDate')) }}
                                          </div>
                                      </div>
                                      <div class="form-group">
                                          <label class="col-lg-2 control-label">End Date:</label>
                                          <div class="col-lg-10">
                                              {{ Form::text('EDate',(isset($EDate)?$EDate:''), array('class' => 'form-control form-control-inline input-medium default-date-picker', 'id' => 'EDate')) }}
                                          </div>
                                      </div>
                                     <div class="form-group">
                                  ...
                                  <fieldset title="Step 3" class="step" id="default-step-2" >
                                      <legend style="color:red;"> Records between given dates Will be Deleted from database!!</legend>
                                      <?php $i=1;?>
          ...
          $('#default').stepy({
              ...
              if (step == '0'){
                    if($('#uploadFile').val() == ""){
                      ...
                      conf = confirm('WARNNING: The Records from '+document.getElementById('SDate').value+' to '+document.getElementById('EDate').value+'  Will be Deleted from database!!');
                      if(conf == false){
                        return false;
                      }else{
                         $('#default').submit();
        ...
        if(arr != ''){
          while(i<arr.length){
            $('#db_field_'+i).val($('#field_db_'+i).val());
              if($('#field_db_'+i).val() == ''){
                alert('Plese Select db fields against your fil fields!');
                $('#default-next-0').click();
                return false;
              }
            i++;
