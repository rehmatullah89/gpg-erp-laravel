              ...
              <section class="panel">
                <header class="panel-heading" style="background: white !important;  border-color: solid grey; color:grey;">
                  ...
                  @if (isset($errors) && ($errors->any()))
                          ...
                          @endif
                 {{ Form::open(array('before' => 'csrf' ,'url'=>route('parts/index'), 'files'=>true, 'method' => 'post')) }}
                            ...
                            <tbody>
                              <tr>
                                  ...
                                  <td data-title="Filter:">
                                    {{Form::select('Filter', array(''=>'Select Filter','part_number'=>'Part #','model_number'=>'Model #','serial_number'=>'Serial #','gpg_field_material_type_id'=>'Type'), null, ['id' => 'Filter', 'class'=>'form-control m-bot15'])}}
                                  </td>
              ...
              <tbody class="cf">
                @foreach($query_data as $row)
                  <tr>
                    ...
                    <td>
                      <a href="{{URL::route('parts.edit', array('id'=>$row['id']))}}">
                          {{Form::button('<i class="fa fa-pencil"></i>', array('class' => 'btn btn-primary btn-xs'))}}
                      </a> 
                      {{ Form::open(array('method' => 'DELETE','id'=>'myForm'.$row['id'].'','style'=>'display:inline; margin:0px; padding:0px;', 'route' => array('parts.destroy', $row['id']))) }}
...
$('button[name=switch_material]').click(function(){
 ...
 var vl = $(this).attr('value');
  $.ajax({
      url: "{{URL('ajax/switchPartStatus')}}",
      data: {
        'id' : id,
        'status' : vl
      },
        success: function (data) {
        alert("Part Status successfully updated!");
        location.reload();
      },
  });
});
